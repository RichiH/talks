...
\begin{frame}
	...
	\begin{itemize}
		...
		\item Advanced ruleset for content distribution
			%%%%%%%%% TODO extra slide with examples when talks' an hour in duration
...
\begin{frame}
	...
	\begin{itemize}
		\item Import podcasts, videos, slides, and other media
		\item Built-in podcatching client (very neat)
		\item Sync or export to consumption devices
		\item Consume media
		\item Drop consumed media content from annex
		\item (Feature to propagate deletions is upcoming)
	\end{itemize}
\end{frame}
...
\subsection{One-slide summary}

\begin{frame}
	\frametitle{Why metamonger?}
	\begin{itemize}
		\item People don't seem to care about persistent file metadata enough to actually do something about it
		\item See discussion on if to store \texttt{mtime} etc in Git...
		\item \texttt{metastore} exists and was written to be used with Git, but
		\begin{itemize}
			\item Binary storage is not merge-safe
			\item Author has no interest to fix this
			\item ...
			\end{itemize}
		\item \texttt{metamonger} uses plain text (line-wise JSON) storage
		\item Will always merge nicely and integrate well with Git
	\end{itemize}
\end{frame}
